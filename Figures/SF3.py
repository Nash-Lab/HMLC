...
data_name = 'uni_2D_diff_mean_fitness.npz'
...
with np.load(data_path+data_name,allow_pickle=True) as data:
    #for k in data.keys():
        diff_mean_fitness_median_shuffle_and_error_calc = data['diff_mean_fitness_median_shuffle_and_error_calc']
        diff_mean_fitness_median_shuffle_and_error_calc = diff_mean_fitness_median_shuffle_and_error_calc.reshape(diff_mean_fitness_median_shuffle_and_error_calc.shape[0],51,51)
        HSs = data['HSs'][0,:,0,0,0].reshape(51,51)
        Ns = data['Ns'][0,:,0,0,0].reshape(51,51)

bias = diff_mean_fitness_median_shuffle_and_error_calc[1]
var = diff_mean_fitness_median_shuffle_and_error_calc[2]
bias_plus_var = diff_mean_fitness_median_shuffle_and_error_calc[3]

sigma_y = 2.4662062487003333
...
for s in ['eq_equidistant','contour_crit_equidistant','n_crit','h_crit']:
    with np.load(data_path+f'{data_name.split("_")[0]}_{s}_out.npz',allow_pickle=True) as data:
        #for k in data.keys():
            temp = data['ys_resorted']
            temp = np.abs(temp-temp[:1])/2.4662062487003333
            temp = np.median(temp.mean(axis=-1),axis=[2,3])
...
caption_A = plt.text(1.05, 0.5, r'$\displaystyle\frac{\sum\limits_{i=1}^{N^{train}}\left|\mu_{y,i}-\mu_{z,i}\right|}{\sigma_{y}\cdot N^{train}}$', horizontalalignment='right', fontsize=fontsize_sup,
...
caption_B = plt.text(0, 0.5, r'$\displaystyle\frac{\sum\limits_{i=1}^{N^{train}}\left|\mu_{y,i}-\bar{y}_{i}\right|}{\sigma_{y}\cdot N^{train}}$', horizontalalignment='left', fontsize=fontsize_sup,
...
caption_C = plt.text(1, 0.5, r'$\displaystyle\frac{\sum\limits_{i=1}^{N^{train}}\left|\mu_{y,i}-\bar{z}_{i}\right|}{\sigma_{y}\cdot N^{train}}$', horizontalalignment='right', fontsize=fontsize_sup,
...
#%% Save
plt.savefig('SF3'+".svg",dpi = 300, format="svg",transparent=True)
plt.savefig('SF3'+'.png',dpi = 400, format='png')